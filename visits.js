...
function renderRow(v = {}, animate = true) {
    ...
    // تم إزالة الكتابة الخافتة (Placeholders) وجعل التواريخ للقراءة فقط
    mainRow.innerHTML = `
        <td class="col-select">
...
async function saveSingleRow(rowId) {
    ...
    // تحديث تاريخ آخر تعديل ليوم التعديل مباشرة على الواجهة
    const today = getTodayDate();
...
// نقل إلى الفرص البيعية في حال اختيار الحالة تأهيل
async function handleStatusChange(selectElem, rowId) {
    ...
    if (newStatus === 'تأهيل لفرصة') {
        try {
            ...
            // الحفظ في مجموعة (الفرص البيعية)
            await setDoc(doc(db, "opportunities", rowId), visitData);
            
            // الحذف من مجموعة (الزيارات)
