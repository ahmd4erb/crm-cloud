...
// ==========================================
// 2. الاستماع اللحظي للبيانات السحابية
// ==========================================
function listenToVisits() {
    ...
    onSnapshot(visitsRef, (snapshot) => {
        ...
        // عدم إعادة التحديث إن كان المستخدم يكتب حالياً لمنع فقدان التركيز
...
function renderRow(v = {}, animate = true) {
    ...
    mainRow.innerHTML = `
        ...
        <td class="col-company"><input type="text" class="excel-input comp-input" value="${v.comp || ''}" placeholder="اسم الشركة"></td>
        <td class="col-address"><input type="text" class="excel-input address-input" value="${v.address || ''}" placeholder="العنوان"></td>
        <td class="col-manager"><input type="text" class="excel-input mgr-input" value="${v.mgr || ''}" placeholder="المسؤول"></td>
        <td class="col-mobile">
            <div class="phone-cell-container">
                <input type="text" class="excel-input mob-input" value="${v.mob || ''}" placeholder="الجوال">
                ${v.mob ? `<a href="https://wa.me/${v.mob.replace(/[^0-9]/g, '')}" target="_blank" class="whatsapp-icon-btn"><i class="fab fa-whatsapp"></i></a>` : ''}
            </div>
        </td>
        <td class="col-email"><input type="text" class="excel-input email-input" value="${v.email || ''}" placeholder="الإيميل"></td>
        <td class="col-record"><input type="text" class="excel-input record-input" value="${v.record || ''}" placeholder="السجل"></td>
        <td class="col-date"><input type="date" class="excel-input visit-date-val" value="${visitDate}"></td>
        <td class="col-service"><input type="text" class="excel-input service-input" value="${v.curServ || ''}" placeholder="الخدمة"></td>
        <td class="col-val"><input type="number" class="excel-input opp-val readonly-input" value="${v.oppValue || ''}" placeholder="0" readonly style="color:var(--accent-blue); font-weight:800;"></td>
        <td class="col-notes">
        ...
        <td class="col-status">
            <select class="excel-input status-select ${getStatusClass(v.status)}" onchange="handleStatusChange(this, '${rowId}')">
                ...
                <option value="مكتملة" ${v.status === 'مكتملة' ? 'selected' : ''}>مكتملة</option>
        ...
        <td class="col-edit">
            <div class="edit-date-container">
                <span class="edit-date-d">${v.editDate || '-'}</span>
                <input type="hidden" class="edit-date-val" value="${v.editDate || ''}">
            </div>
        </td>
        <td class="col-owner"><input type="text" class="excel-input owner-input" value="${v.owner || ''}" placeholder="المالك"></td>
    `;
    ...
    // بناء قائمة المنتجات الفرعية إن وجدت
...
// ==========================================
// 5. حفظ وحذف الزيارات على السحابة
// ==========================================
async function saveSingleRow(rowId) {
    ...
    const visitData = {
        ...
        editDate: row.querySelector('.edit-date-val').value || getTodayDate(),
