...
const CUSTOMERS_STORAGE_KEY = 'asgate_customers_cloud'; // تم التحديث ليجلب من Firebase في sales.html 
let saveTimeout;
...
// تم نقل البحث الحقيقي إلى sales.html ليجلب من Firebase مباشرة
// هذه الدالة للتوافق فقط - البحث الفعلي يتم في الموديول داخل sales.html
function searchCustomerInModal(el) {
    // سيتم استدعاء الدالة من sales.html المحدثة التي تبحث في Firebase
    if(window.searchCustomerInModal && window.searchCustomerInModal !== searchCustomerInModal){
        return window.searchCustomerInModal(el);
    }
    ...
    try {
        const customers = JSON.parse(localStorage.getItem('asgate_customers_final_v2') || '[]');
        const filtered = customers.filter(c => {
            const cName = (c.comp || "").toLowerCase();
            const code = (c.code || "").toLowerCase();
            const crMain = String(c.cr1 || c.cr || "").toLowerCase();
            const crSub = String(c.cr2 || "").toLowerCase();
            return cName.includes(query) || code.includes(query) || crMain.includes(query) || crSub.includes(query);
        });
        resDiv.innerHTML = filtered.map(c => {
            const displayCr = c.cr1 || c.cr || c.cr2 || 'بدون سجل';
            return `<div onclick="selectCustomer('${c.comp || ''}', '${displayCr}')"><i class="far fa-building"></i> ${c.comp || 'بدون اسم'} - ${displayCr} (${c.code||''})</div>`;
        }).join('');
